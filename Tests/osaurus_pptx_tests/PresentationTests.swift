...
struct UnitConversionTests {
    ...
    func inchesToEMU() {
        ...
        #expect(Units.inchesToEMU(2.5) == 2286000)
    ...
    func xmlEscapeTest() {
        ...
        #expect(xmlEscape("\"") == "&quot;")
        #expect(xmlEscape("'") == "&apos;")
        #expect(xmlEscape("a & b < c > d \"e\" 'f'") == "a &amp; b &lt; c &gt; d &quot;e&quot; &apos;f&apos;")
    }
...
struct ModelTests {
    ...
    func elementPositionEMU() {
        ...
        #expect(pos.yEMU == 1828800)
        #expect(pos.widthEMU == 2743200)
        #expect(pos.heightEMU == 3657600)
    }
}
...
struct ToolExecutionTests {
    ...
    func createPresentationCustom() {
        var presentations: [String: Presentation] = [:]
        let tool = CreatePresentationTool()
        let result = tool.run(args: "{\"title\": \"Custom\", \"layout\": \"4:3\", \"theme\": \"dark\"}", presentations: &presentations)
        let json = parseJSON(result)
    ...
    func addSlide() {
        ...
        let result = slideTool.run(args: "{\"presentation_id\": \"\(presId)\", \"layout\": \"title\"}", presentations: &presentations)
        let json = parseJSON(result)
    ...
    func addSlideInvalidId() {
        ...
        let result = tool.run(args: "{\"presentation_id\": \"nonexistent\"}", presentations: &presentations)
        let json = parseJSON(result)
        #expect(json?["error"] != nil)
    }
    ...
    func addTextInvalidSlide() {
        ...
        let tool = AddTextTool()
        let result = tool.run(args: "{\"presentation_id\": \"\(presId)\", \"slide_number\": 99, \"text\": \"test\"}", presentations: &presentations)
        let json = parseJSON(result)
        #expect(json?["error"] != nil)
    }
    ...
    func addShapeInvalidType() {
        ...
        let tool = AddShapeTool()
        let result = tool.run(args: "{\"presentation_id\": \"\(presId)\", \"slide_number\": 1, \"shape_type\": \"nonexistent\"}", presentations: &presentations)
        let json = parseJSON(result)
        #expect(json?["error"] != nil)
    ...
    func setSlideBackground() {
        ...
        // Solid
        let solidResult = tool.run(args: "{\"presentation_id\": \"\(presId)\", \"slide_number\": 1, \"color\": \"1F3864\"}", presentations: &presentations)
    ...
    func deleteSlide() {
        ...
        let result = tool.run(args: "{\"presentation_id\": \"\(presId)\", \"slide_number\": 1}", presentations: &presentations)
        let json = parseJSON(result)
    ...
    func getPresentationInfo() {
        ...
        _ = textTool.run(args: "{\"presentation_id\": \"\(presId)\", \"slide_number\": 1, \"text\": \"Hello\"}", presentations: &presentations)

        let tool = GetPresentationInfoTool()

        // Basic
        let basicResult = tool.run(args: "{\"presentation_id\": \"\(presId)\"}", presentations: &presentations)
        let basicJSON = parseJSON(basicResult)
        #expect(basicJSON?["slide_count"] as? Int == 1)
        #expect(basicJSON?["title"] != nil)

        // Detailed
        let detailResult = tool.run(args: "{\"presentation_id\": \"\(presId)\", \"include_details\": true}", presentations: &presentations)
    ...
    func fullWorkflow() {
        ...
        // Create
        let createTool = CreatePresentationTool()
        let createResult = createTool.run(args: "{\"title\": \"Workflow Test\", \"theme\": \"corporate\"}", presentations: &presentations)
        let presId = parseJSON(createResult)?["presentation_id"] as! String

        // Add slide
        let slideTool = AddSlideTool()
        _ = slideTool.run(args: "{\"presentation_id\": \"\(presId)\", \"layout\": \"title_content\"}", presentations: &presentations)

        // Add text
        let textTool = AddTextTool()
        _ = textTool.run(args: "{\"presentation_id\": \"\(presId)\", \"slide_number\": 1, \"text\": \"Title Text\", \"bold\": true, \"font_size\": 36}", presentations: &presentations)

        // Add shape
        let shapeTool = AddShapeTool()
        _ = shapeTool.run(args: "{\"presentation_id\": \"\(presId)\", \"slide_number\": 1, \"shape_type\": \"rect\", \"fill_color\": \"4472C4\"}", presentations: &presentations)
        ...
        let info = infoTool.run(args: "{\"presentation_id\": \"\(presId)\", \"include_details\": true}", presentations: &presentations)
    ...
    // Helper
    private func createPresentationWithSlide(_ presentations: inout [String: Presentation]) -> (String, String) {
        let createTool = CreatePresentationTool()
        ...
        let slideTool = AddSlideTool()
        let slideResult = slideTool.run(args: "{\"presentation_id\": \"\(presId)\"}", presentations: &presentations)
...
struct XMLGenerationTests {
    ...
    func textBoxXML() {
        ...
        let text = TextElement(text: "Sample Text", position: pos, fontSize: 24, fontFace: "Arial", fontColor: "FF0000", bold: true)
    ...
    func shapeXML() {
        ...
        let shape = ShapeElement(shapeType: .roundRect, position: pos, fillColor: "4472C4", text: "Click Me", textColor: "FFFFFF")
...
struct PPTXRoundTripTests {
    ...
    func writeReadRoundTrip() throws {
        ...
        let slide = Slide()
        let pos = ElementPosition(x: 1.0, y: 1.0, width: 8.0, height: 1.5)
        let text = TextElement(text: "Preserved Text", position: pos, fontSize: 20, fontFace: "Calibri", fontColor: "333333", bold: true)
        slide.elements.append(text)
        pres.slides.append(slide)
    ...
    func writeMultipleElements() throws {
        ...
        let shape = ShapeElement(shapeType: .rect, position: shapePos, fillColor: "4472C4", text: "Box Text")
