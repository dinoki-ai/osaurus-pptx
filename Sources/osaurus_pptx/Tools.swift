...
struct AddTextTool {
    ...
    func run(args: String, presentations: inout [String: Presentation]) -> String {
        ...
        guard input.slide_number >= 1 && input.slide_number <= pres.slides.count else {
            return jsonError("Invalid slide number: \(input.slide_number). Presentation has \(pres.slides.count) slides.")
        }
...
struct AddImageTool {
    ...
    func run(args: String, presentations: inout [String: Presentation]) -> String {
        ...
        guard validExts.contains(ext) else {
            return jsonError("Unsupported image format: \(ext). Supported: \(validExts.joined(separator: ", "))")
        }
...
struct AddShapeTool {
    ...
    func run(args: String, presentations: inout [String: Presentation]) -> String {
        ...
        guard let shapeType = ShapeType(rawValue: input.shape_type) else {
            let validTypes = ["rect", "round_rect", "ellipse", "triangle", "diamond", "pentagon", "hexagon", "octagon", "star4", "star5", "star6", "right_arrow", "left_arrow", "up_arrow", "down_arrow", "heart", "cloud", "lightning", "line", "parallelogram", "trapezoid"]
            return jsonError("Invalid shape type: \(input.shape_type). Valid: \(validTypes.joined(separator: ", "))")
        }
...
struct AddTableTool {
    ...
    func run(args: String, presentations: inout [String: Presentation]) -> String {
        guard let data = args.data(using: .utf8),
            ...
            return jsonError("Invalid arguments. Required: presentation_id, slide_number, rows (2D array)")
        }
...
struct AddChartTool {
    ...
    func run(args: String, presentations: inout [String: Presentation]) -> String {
        guard let data = args.data(using: .utf8),
            ...
            return jsonError("Invalid arguments. Required: presentation_id, slide_number, chart_type, categories, series")
        }
        ...
        guard let chartType = ChartType(rawValue: input.chart_type) else {
            return jsonError("Invalid chart type: \(input.chart_type). Valid: bar, column, line, pie, doughnut")
        }
...
struct SetSlideBackgroundTool {
    ...
    func run(args: String, presentations: inout [String: Presentation]) -> String {
        guard let data = args.data(using: .utf8),
            ...
            return jsonError("Invalid arguments. Required: presentation_id, slide_number, and either color or gradient_color1+gradient_color2")
        }
        ...
        if let color = input.color {
            ...
            slide.background = SlideBackground(type: .gradient(color1: c1, color2: c2, angle: input.gradient_angle ?? 270))
        } else {
            return jsonError("Provide either 'color' for solid background or 'gradient_color1' and 'gradient_color2' for gradient")
        }
...
struct GetPresentationInfoTool {
    ...
    func run(args: String, presentations: inout [String: Presentation]) -> String {
        ...
        if input.include_details == true {
            ...
            for (idx, slide) in pres.slides.enumerated() {
                ...
                slidesJSON += "{\"number\": \(idx + 1), \"layout\": \"\(slide.layoutType.rawValue)\", \"background\": \"\(bgDesc)\", \"element_count\": \(slide.elements.count), \"elements\": \(elementsJSON)}"
            }
            slidesJSON += "]"
            ...
            for (idx, slide) in pres.slides.enumerated() {
                ...
                slidesSummary += "{\"number\": \(idx + 1), \"layout\": \"\(slide.layoutType.rawValue)\", \"element_count\": \(slide.elements.count)}"
            }
            slidesSummary += "]"
    ...
    private func describeElement(_ element: SlideElement) -> String {
        if let text = element as? TextElement {
            ...
            return "{\"type\": \"text\", \"id\": \"\(text.elementId)\", \"text\": \"\(jsonEscape(preview))\", \"font_size\": \(text.fontSize), \"bold\": \(text.bold), \"x\": \(text.position.x), \"y\": \(text.position.y), \"width\": \(text.position.width), \"height\": \(text.position.height)}"
        } else if let image = element as? ImageElement {
            return "{\"type\": \"image\", \"id\": \"\(image.elementId)\", \"path\": \"\(jsonEscape(image.sourcePath))\", \"x\": \(image.position.x), \"y\": \(image.position.y), \"width\": \(image.position.width), \"height\": \(image.position.height)}"
        } else if let shape = element as? ShapeElement {
            return "{\"type\": \"shape\", \"id\": \"\(shape.elementId)\", \"shape_type\": \"\(shape.shapeType.rawValue)\", \"x\": \(shape.position.x), \"y\": \(shape.position.y), \"width\": \(shape.position.width), \"height\": \(shape.position.height)}"
        } else if let table = element as? TableElement {
            return "{\"type\": \"table\", \"id\": \"\(table.elementId)\", \"rows\": \(table.rows.count), \"columns\": \(table.rows.first?.count ?? 0)}"
        } else if let chart = element as? ChartElement {
            return "{\"type\": \"chart\", \"id\": \"\(chart.elementId)\", \"chart_type\": \"\(chart.chartType.rawValue)\", \"series_count\": \(chart.series.count)}"
        }
        return "{\"type\": \"unknown\"}"
    }
}
