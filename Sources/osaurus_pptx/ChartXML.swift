...
enum ChartXMLGenerator {
    ...
    static func generateChartXML(chart: ChartElement) -> String {
        let titleXML = chart.chartTitle.map { generateTitleXML($0) } ?? "<c:autoTitleDeleted val=\"1\"/>"
        ...
        let legendXML = chart.showLegend ? """
          <c:legend>
            <c:legendPos val="b"/>
            <c:overlay val="0"/>
          </c:legend>
    ...
    private static func generateCatRef(categories: [String]) -> String {
        var catXML = "<c:cat><c:strRef><c:f>Sheet1!$A$2:$A$\(categories.count + 1)</c:f><c:strCache><c:ptCount val=\"\(categories.count)\"/>"
    ...
    private static func generateValRef(values: [Double], seriesIndex: Int, count: Int) -> String {
        ...
        var valXML = "<c:val><c:numRef><c:f>Sheet1!$\(colLetter)$2:$\(colLetter)$\(count + 1)</c:f><c:numCache><c:formatCode>General</c:formatCode><c:ptCount val=\"\(values.count)\"/>"
