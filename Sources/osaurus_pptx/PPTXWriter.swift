...
enum PPTXWriter {
    ...
    static func write(presentation: Presentation, to outputPath: String) throws {
        ...
        for imageFile in imageFiles {
            ...
            if let data = imageFile.sourcePath.starts(with: "/") ? FileManager.default.contents(atPath: imageFile.sourcePath) : nil {
        ...
        for chartFile in chartFiles {
            ...
            try writeFile(ChartXMLGenerator.generateChartRels(), to: "\(tempDir)/ppt/charts/_rels/chart\(chartFile.chartIndex).xml.rels")
        }

        // Write theme
        try writeFile(ThemeXMLGenerator.generateThemeXML(theme: presentation.theme), to: "\(tempDir)/ppt/theme/theme1.xml")

        // Write slide master
        try writeFile(ThemeXMLGenerator.generateSlideMasterXML(theme: presentation.theme), to: "\(tempDir)/ppt/slideMasters/slideMaster1.xml")
        try writeFile(ThemeXMLGenerator.generateSlideMasterRels(), to: "\(tempDir)/ppt/slideMasters/_rels/slideMaster1.xml.rels")

        // Write slide layout
        try writeFile(ThemeXMLGenerator.generateSlideLayoutXML(layoutType: .blank), to: "\(tempDir)/ppt/slideLayouts/slideLayout1.xml")
        try writeFile(ThemeXMLGenerator.generateSlideLayoutRels(), to: "\(tempDir)/ppt/slideLayouts/_rels/slideLayout1.xml.rels")
        ...
        for (slideIdx, slide) in presentation.slides.enumerated() {
            ...
            var slideRels: [SlideRelationship] = [
                SlideRelationship(rId: "rId1", type: OOXML.relTypeSlideLayout, target: "../slideLayouts/slideLayout1.xml")
            ]
            ...
            for element in slide.elements {
                if element is ImageElement {
                    ...
                    slideRels.append(SlideRelationship(
        ...
        try writeFile(generatePresentationXML(presentation: presentation), to: "\(tempDir)/ppt/presentation.xml")

        // Write presentation.xml.rels
        try writeFile(generatePresentationRelsXML(presentation: presentation), to: "\(tempDir)/ppt/_rels/presentation.xml.rels")

        // Write [Content_Types].xml
        try writeFile(generateContentTypesXML(presentation: presentation, imageFiles: imageFiles, chartCount: chartFiles.count), to: "\(tempDir)/[Content_Types].xml")

        // Write _rels/.rels
        try writeFile(generateRootRelsXML(), to: "\(tempDir)/_rels/.rels")

        // Write docProps/core.xml
        try writeFile(generateCorePropsXML(presentation: presentation), to: "\(tempDir)/docProps/core.xml")
        ...
        let result = try runProcess("/usr/bin/zip", arguments: ["-r", "-q", outputPath, "."], currentDirectory: tempDir)
    ...
    private static func generatePresentationRelsXML(presentation: Presentation) -> String {
        var rels = ""
        rels += "  <Relationship Id=\"rIdMaster1\" Type=\"\(OOXML.relTypeSlideMaster)\" Target=\"slideMasters/slideMaster1.xml\"/>\n"
        rels += "  <Relationship Id=\"rIdTheme1\" Type=\"\(OOXML.relTypeTheme)\" Target=\"theme/theme1.xml\"/>\n"

        for (slideIdx, _) in presentation.slides.enumerated() {
            let num = slideIdx + 1
            rels += "  <Relationship Id=\"rIdSlide\(num)\" Type=\"\(OOXML.relTypeSlide)\" Target=\"slides/slide\(num).xml\"/>\n"
        }
    ...
    private static func generateContentTypesXML(presentation: Presentation, imageFiles: [(sourcePath: String, targetName: String, ext: String)], chartCount: Int) -> String {
        var overrides = ""
        overrides += "  <Override PartName=\"/ppt/presentation.xml\" ContentType=\"\(OOXML.contentTypePresentationML)\"/>\n"
        overrides += "  <Override PartName=\"/ppt/slideMasters/slideMaster1.xml\" ContentType=\"\(OOXML.contentTypeSlideMaster)\"/>\n"
        overrides += "  <Override PartName=\"/ppt/slideLayouts/slideLayout1.xml\" ContentType=\"\(OOXML.contentTypeSlideLayout)\"/>\n"
        overrides += "  <Override PartName=\"/ppt/theme/theme1.xml\" ContentType=\"\(OOXML.contentTypeTheme)\"/>\n"
        overrides += "  <Override PartName=\"/docProps/core.xml\" ContentType=\"\(OOXML.contentTypeCoreProps)\"/>\n"

        for (slideIdx, _) in presentation.slides.enumerated() {
            let num = slideIdx + 1
            overrides += "  <Override PartName=\"/ppt/slides/slide\(num).xml\" ContentType=\"\(OOXML.contentTypeSlide)\"/>\n"
        }

        if chartCount > 0 {
            for i in 1...chartCount {
                overrides += "  <Override PartName=\"/ppt/charts/chart\(i).xml\" ContentType=\"\(OOXML.contentTypeChart)\"/>\n"
            }
        }
        ...
        for img in imageFiles {
            ...
            if !seenExts.contains(ext) {
                seenExts.insert(ext)
                imageDefaults += "  <Default Extension=\"\(ext)\" ContentType=\"\(imageContentType(for: ext))\"/>\n"
            }
        }
