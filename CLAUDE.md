# Osaurus Presentation - Osaurus Plugin
...
```json
{
  "plugin_id": "dev.example.osaurus-pptx",
...
```bash
# Package the plugin
osaurus tools package dev.example.osaurus-pptx 0.1.0

# Install locally
osaurus tools install ./dev.example.osaurus-pptx-0.1.0.zip
...
```bash
swift build -c release && osaurus tools package dev.example.osaurus-pptx 0.1.0 && osaurus tools install ./dev.example.osaurus-pptx-0.1.0.zip
```
...
codesign --force --options runtime --timestamp \
  ...
  .build/release/libosaurus-pptx.dylib
```
